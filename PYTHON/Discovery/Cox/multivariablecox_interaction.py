...
def fit_lasso_cox_int(df,penalty):
    Event = "Inc_Fx_Inc_fx_hip_bin_cens"
    Time = "Inc_Fx_Inc_fx_hip_days_to_cens"
    covs = df.columns.difference([Event, Time,"Age","Sex"])
    df = pd.get_dummies(df,columns=covs, drop_first=True)
    covs = df.columns.difference(["Sex","Age",Event, Time])
    formula = "Sex+"
    for cov in covs:
...
def main(  output_dir, Group_ID, penalty):
    ...
    usecols=['LopNr', 'Sub_Cohort',"Inc_Fx_Inc_fx_hip_bin_cens","Inc_Fx_Inc_fx_hip_days_to_cens",
    "Age","Sex",'Prev_fx_occ_n_13y_bin', 'Prev_fx_occ_n_10y_bin', 'Prev_fx_occ_n_5y_bin', 'Prev_fx_occ_n_2y_bin','Prev_fx_occ_n_1y_bin'],engine = "pyarrow") 
    df = df.set_index("LopNr")
    df = df[df["Sub_Cohort"] == "Discovery"]  # Filter data

###################### read csv file with singificnt covartiates
    file_column_mapping = pd.read_csv('/path/to/private_feature_maps/Cox_int_groups_new.csv', sep=",") 
    ...
    for folder in folder_paths:
        for file in files:
            ...
            if (os.path.exists(file_path)):
                print(file_path)
                column_name = file_column_mapping["Covariate_tested"][file_column_mapping ["File"]==file]
    ...
    ###################### FIT THE MODEL
    delayed_fit = delayed(fit_lasso_cox_int)(df, penalty)
    ...
    #######################################################  Saving model
    with open (os.path.join(output_dir, "Multi_int_CoxLassoGroup_" + str(Group_ID) + ".pkl"),"wb") as f:
        pickle.dump(model,f)
    
    print("Model saved at:", os.path.join(output_dir, "Multi_int_CoxLassoGroup_" + str(Group_ID) + ".pkl"))




#################### EXUCUTING THE SCRIPT
