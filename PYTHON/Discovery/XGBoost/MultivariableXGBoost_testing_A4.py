...
def save_feature_importance(booster, predictors, output_dir):
    ...
    feature_importance_df.to_csv(os.path.join(output_dir, 'A4_important_features.csv'), index=False)
    print(f"Important features saved at: {os.path.join(output_dir, 'A4_important_features.csv')}")
...
def main(input_csv, model_path, output_dir, importance_threshold=5):
    ...
    df2['actual_time_to_fracture'] = df2["xgb_response"]  # New line
    ...
    # Saving preditions
    df3.to_csv(os.path.join(output_dir, "A4_predictions.csv"), index=False)
    print("Predictions saved at:", os.path.join(output_dir, "A4_predictions.csv"))
