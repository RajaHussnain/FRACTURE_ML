...
def save_feature_importance(booster, predictors, output_dir):
    ...
    feature_importance_df.to_csv(os.path.join(output_dir, 'A5_important_features.csv'), index=False)
    print(f"Important features saved at: {os.path.join(output_dir, 'A5_important_features.csv')}")
...
def main(input_csv, model_path, output_dir, importance_threshold=5):
    ...
    # Saving preditions
    df3.to_csv(os.path.join(output_dir, "A5_predictions.csv"), index=False)
    print("Predictions saved at:", os.path.join(output_dir, "A5_predictions.csv"))
