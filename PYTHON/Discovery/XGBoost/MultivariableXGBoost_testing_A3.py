...
def save_feature_importance(booster, predictors, output_dir):
    ...
    #important_features = feature_importance_df[feature_importance_df['Importance_Score'] >= importance_threshold]

    #important_features.to_csv(os.path.join(output_dir, 'A4_important_features.csv'), index=False)
    feature_importance_df.to_csv(os.path.join(output_dir, 'A3_important_features.csv'), index=False)
    print(f"Important features saved at: {os.path.join(output_dir, 'A3_important_features.csv')}")

    #return important_features['Feature'].tolist()
    return feature_importance_df['Feature'].tolist()
    

def main(input_csv, model_path, output_dir, importance_threshold=5):
    ...
    # Saving preditions
    df3.to_csv(os.path.join(output_dir, "A3_predictions.csv"), index=False)
    print("Predictions saved at:", os.path.join(output_dir, "A3_predictions.csv"))
