...
def add_content_to_doc(doc,col_true, col_pred, heading,optimal_cutoff):
...
def main(MODEL_NAME):
    ...
    folder = "/path/to/project/Output/MultivariableCox/Lasso"
    ...
    input_csv = "/path/to/project/Output/MultivariableCox/Lasso/"+MODEL_NAME+".parquet"
    ...
    covs = df_train.columns.difference(['Inc_Fx_Inc_fx_hip_bin_cens', 'Inc_Fx_Inc_fx_hip_days_to_cens',"Age"])
    df_val = pd.get_dummies(df_val, columns=covs,drop_first=True)
    df_train = pd.get_dummies(df_train, columns=covs,drop_first=True)



    doc = Document()
    doc.add_heading("Cox Model Evaluation", level=1)

    surv=  model.predict_survival_function(df_val)
    surv_train = model.predict_survival_function(df_train)

    columns_nan = surv.columns[surv.isna().any()].tolist()
    surv = surv.dropna(axis=1)
    surv_train = surv_train.dropna(axis=1)

    y_true = df_val    [["Inc_Fx_Inc_fx_hip_bin_cens"]]
    df_pred = y_true

    y_true_train = df_train    [["Inc_Fx_Inc_fx_hip_bin_cens"]]
    df_pred_train = y_true_train
    ...
    for N in range(1,11):
        ...
        y_test_event = np.delete(y_test_event, columns_nan)
        ...
        # Apply the optimal cut-off to convert probabilities to binary predictions
 ...
 # confusion
    ...
    plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/ROC_"+MODEL_NAME+".pdf", format='pdf', dpi=1200)  # High-quality PDF format
    plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/ROC_"+MODEL_NAME+".eps", format='eps', dpi=1200)
    plt.close()
    ...
    plt.plot(range(1, 11), sens, marker="o", label="Sensitivity")
    plt.plot(range(1, 11), spec, marker="o", label="Specificity")
    plt.plot(range(1, 11), PPV, marker="o", label="PPV")
    plt.plot(range(1, 11), NPV, marker="o", label="NPV")
    plt.xlabel("Years")
    plt.ylabel("Metric Value")
    plt.title("AUC, Sensitivity, Specificity, PPV and NVP Over 10 Years")
    plt.ylim(0,1)
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/AUC_"+MODEL_NAME+".pdf", format='pdf', dpi=1200)  # High-quality PDF format
    plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/AUC_"+MODEL_NAME+".eps", format='eps', dpi=1200)
    ...
    for N in range(1,11):
        ...
        metrics_buffer = io.BytesIO()
        plt.savefig(metrics_buffer, format="png")
        plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/Calibration"+MODEL_NAME+"_Year"+str(N)+".pdf", format='pdf', dpi=1200)  # High-quality PDF format
        plt.savefig("/path/to/project/Output/MultivariableCox/Dev_results/Calibration"+MODEL_NAME+"_Year"+str(N)+".eps", format='eps', dpi=1200)
    ...
    doc.save("/path/to/project/Output/MultivariableCox/Dev_results/"+MODEL_NAME+"youden.docx")
    

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train XGBoost model with Dask and save the results.")
    parser.add_argument("--MODEL_NAME", type=str, required=True, help="Path to the input CSV file.")
