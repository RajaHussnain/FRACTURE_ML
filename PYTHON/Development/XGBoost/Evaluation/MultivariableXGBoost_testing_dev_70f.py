...
def plot_feature_importance(model, feature_names, output_dir):
    ...
    feature_importance_df.to_csv(os.path.join(output_dir, 'dev_70f_important_features.csv'), index=False)
    print(f"Important features saved at: {os.path.join(output_dir, 'dev_70f_important_features.csv')}")
    ...
    plt.savefig(os.path.join(output_dir, 'dev_70f_feature_importance.png'))
    plt.close()
    print(f"Feature importance plot saved at: {os.path.join(output_dir, 'dev_70f_feature_importance.png')}")
...
def calculate_calibration(y_true, y_pred, years, output_dir):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, 'dev_70f_calibration_curve.png'))
    plt.close()
    print(f"Combined calibration plot saved at: {os.path.join(output_dir, 'dev_70f_calibration_curve.png')}")
...
def calculate_concordance_index(y_true, y_pred, years, output_dir, n_bootstraps=1000, alpha=0.05):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(os.path.join(output_dir, 'dev_70f_concordance_index_per_year.png'))
    plt.close()
    print(f"Concordance index plot saved at: {os.path.join(output_dir, 'dev_70f_concordance_index_per_year.png')}")
...
def main(input_csv, model_path, output_dir):
    ...
    results.to_csv(os.path.join(output_dir, 'dev_70f_predictions.csv'), index=False)
    print(f"Predictions saved at: {os.path.join(output_dir, 'dev_70f_predictions.csv')}")

    # Calculate and save concordance index
    ci = concordance_index(y_true, y_pred)
    with open(os.path.join(output_dir, 'dev_70f_concordance_index.txt'), 'w') as f:
    ...
    plt.savefig(os.path.join(output_dir, 'dev_70f_kaplan_meier.png'))
    plt.close()
    print(f"Kaplan-Meier plot saved at: {os.path.join(output_dir, 'dev_70f_kaplan_meier.png')}")
